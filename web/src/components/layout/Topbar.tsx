  ...
  return (
    ...
    >
      ...
      <input
        ...
        className="h-9 w-full rounded-lg bg-raised pr-14 pl-9 text-sm text-ink ring-1 ring-transparent transition-shadow duration-150 placeholder:text-subtle focus:bg-surface focus:ring-accent"
      />
      <kbd className="absolute top-1/2 right-2.5 -translate-y-1/2 rounded border border-line px-1.5 py-0.5 text-[10px] font-medium text-subtle">
        ⌘K
      </kbd>
    </form>
  );
...
const PresenceChip = () => {
  ...
  return (
    <span
      className="hidden items-center gap-1.5 rounded-lg bg-raised px-2.5 py-1.5 text-xs text-muted md:inline-flex"
      title={connected ? 'Live updates connected' : 'Reconnecting to live updates'}
    >
      <span
        className={clsx('size-1.5 rounded-full', connected ? 'bg-success' : 'bg-warn')}
      />
    ...
    <div className="relative" ref={container}>
      ...
      {open ? (
        <div className="animate-fade absolute right-0 z-40 mt-2 w-60 rounded-xl bg-surface p-3 shadow-xl ring-1 ring-line">
          ...
          <p className="mt-3 rounded-lg bg-raised px-2.5 py-1.5 text-xs text-muted">
...
export const Topbar = ({ onOpenNav }: { onOpenNav: () => void }) => (
  <header className="sticky top-0 z-30 flex h-14 items-center gap-2 border-b border-line bg-surface px-3 sm:gap-3 sm:px-4">
    <button
      type="button"
      onClick={onOpenNav}
