...
```
   Vercel                 Render                    Neon
   ──────                 ──────                    ────
   web (static)  ──────►  API (Docker, WebSocket) ──►  PostgreSQL
   vercel.json            render.yaml                 sslmode=require
```

**The API cannot run on Vercel.** Its serverless functions are request-scoped and terminate once
a response is sent, so they cannot hold an open WebSocket — and the brief rules out long-polling
and SSE as substitutes. Putting the API there would mean abandoning the requirement that carries
25% of the marks. So Vercel serves the client and Render runs the API, which supports long-lived
connections. Neon holds the database because Render's own free Postgres is deleted after 30 days.

Both config files are committed: [`web/vercel.json`](web/vercel.json) and
[`render.yaml`](render.yaml). Render builds the same `server/Dockerfile` the local stack uses, so
what runs in production is what was tested.
...
Create a project, then copy the connection string. It must end with `?sslmode=require`:

```
postgresql://<user>:<password>@ep-xxx-xxx.<region>.aws.neon.tech/velozity?sslmode=require
```

Apply the schema and load the demo data once, from your machine:

```bash
cd server
DATABASE_URL="<the neon url>" npx prisma migrate deploy
DATABASE_URL="<the neon url>" npm run seed
```

### 3. API on Render

New → Blueprint, point it at the repository, and Render reads `render.yaml`. Everything is set
there except three values, which you enter in the dashboard:

| Variable | Value |
|---|---|
| `DATABASE_URL` | the Neon URL from step 2 |
| `CORS_ORIGIN` | `https://velozity-dashboard.vercel.app` |
| `PUBLIC_API_URL` | `https://velozity-api.onrender.com` |

`JWT_ACCESS_SECRET` and `JWT_REFRESH_SECRET` are generated by Render, so they are never written
down. **Do not set `COOKIE_DOMAIN`** — leaving it unset keeps the refresh cookie host-only on the
API domain, which is what a cross-site setup needs.

`SEED_ON_START` stays `false`. A free instance restarts every time it wakes from sleep, and
seeding truncates the demo data, so leaving it on would wipe anything a visitor changed
mid-review.

### 4. Web on Vercel

Import the repository and set **Root Directory to `web`**. `vercel.json` supplies the framework,
build command, output directory and the SPA rewrite, so the only thing to add is one environment
variable:

```
VITE_API_URL = https://velozity-api.onrender.com
```
...
```
sign in                          the seeded accounts work
reload the page                  you stay signed in  ← the cross-site cookie
open two windows, two roles      a status change appears live in both
sign out, sign in again          no stale session
```

The reload is the one that catches a misconfigured deployment. If login succeeds and a refresh
logs you out, the browser refused the refresh cookie. The API will not let that happen quietly:
it validates the cookie combination at boot and refuses to start on a broken one.

```
COOKIE_SAMESITE=lax cross-site  → refuses: the cookie would never be sent
COOKIE_SECURE=false             → refuses: browsers drop SameSite=None without Secure
